...
class sobol:
    ...
    def _dualInteractIndex(self):
        ...
        # Complement to the pairs (The dropped indices on which integration is done)
        compIndex = []
        indexList = np.arange(self.p)
    ...
    def hdmr_0_1(self):
        ...
        for i in range(p):
            # Index of remaining parameters to integrate over
            parIndexList = np.arange(p)
            # permut list I=[]
    ...
    def partVariance(self):
        ...
        for k in range(len(fij)):
            ...
            Dij.append(self.doubleInteg(
                np.multiply(fij[k]**2., pdf_), q[i], q[j]))
