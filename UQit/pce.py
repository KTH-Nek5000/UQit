...
class pce:
    ...
    def _info(self):
        ...
        """
        obligKeyList = ['p', 'distType', 'sampleType',
                        'pceSolveMethod']  # obligatory keys in pceDict
    ...
    def _pceDict_corrector(self):
        ...
        if len(self.distType) != self.p:
            raise ValueError(
                "Provide %d 'distType' for the random parameter" % self.p)
        if len(self.nQList) > 0 and len(self.nQList) != self.p:
            raise ValueError(
                "For 'truncMethod':'TP', nQList of the length p=%d is needed." % self.pceDict['p'])
        if len(self.xi) > 0:  # if xi is provided
            if self.xi.ndim != 2:
                raise ValueError(
                    "Wrong dimension: xi must be a 2D numpy array.")
            else:
                if self.xi.shape[1] != self.p:
                    raise ValueError(
                        "Second dimension of xi should be equal to p=" % self.p)
            ...
            if self.pceDict['p'] != 1:
                ...
                if self.pceDict['pceSolveMethod'] == 'Projection':
                    if self.pceDict['sampleType'] != 'GQ':
                        ...
                        if self.verbose:
                            print(
                                "... Original 'Projection' method for PCE is replaced by 'Regression'.")
        else:
            if self.pceDict['p'] > 1:
                ...
                if self.pceDict['pceSolveMethod'] == 'Regression' and self.pceDict['truncMethod'] != 'TP':
                    if 'LMax' not in self.pceDict:
                        print(
                            "WARNING in pceDict: 'LMax' should be set when Total-Order method is used.")
                        print("Here 'LMax' is set to default value %d" %
                              self.LMax_def)
        ...
        for distType_ in self.distType:
            if distType_ not in self.availDist:
                raise ValueError(
                    "Invalid 'distType'! Availabe list: ", self.availDist)
    ...
    def cnstrct_GQTP_pd(self):
        ...
        if self.verbose:
            ...
            print('...... Method of computing PCE coefficients: %s' %
                  self.pceSolveMethod)
        ...
        fVal_ = self.fVal.reshape(self.nQList, order='F').T
        for k in range(K):
            psi_k = []
            for j in range(p):
        ...
        if self.pceDict['pceSolveMethod'] == 'Regression':
            ...
            for k in range(K):
                ...
                A[:, k] = aij_
            # This is a uniquely determined system
            fCoef = linAlg.myLinearRegress(A, self.fVal)
    ...
    def cnstrct_nonGQTP_pd(self):
        ...
        if self.truncMethod == 'TO':
            ...
            if self.verbose:
                print(
                    '         with LMax=%d as the max polynomial order in each direction.' % LMax)
        if self.pceSolveMethod != 'Regression':
            raise ValueError(
                "only Regression method can be used for PCE with Total Order truncation method.")
        # (1) Preliminaries
...
class pceEval:
    ...
    def eval_pd(self):
        ...
        K = self.K
        for k in range(K):
            a_ = self.coefs[k]*pce.basis(self.kSet[k]
                                         [0], self.xi[0], self.distType[0])
            for i in range(1, p):
...
class convPlot:
    ...
    def pceConvPlot(self):
        ...
        plt.ylabel(
            r'$|\hat{f}_\mathbf{k}|\, ||\Psi_{\mathbf{k}(\mathbf{\xi})}||_2/|\hat{f}_0|$', fontsize=18)
