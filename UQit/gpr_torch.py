...
class SingletaskGPModel_mIn(gpytorch.models.ExactGP):
    ...
    def __init__(self, train_x, train_y, likelihood):
        super(SingletaskGPModel_mIn, self).__init__(
...
class gpr:
        ...
        if 'standardizeYTrain' in self.gprOpts.keys():
            if self.gprOpts['standardizeYTrain']:
                for i in range(self.nResp):
                    ...
                    self.yTrain[:, i] = (
                        self.yTrain[:, i]-self.shift[i])/self.scale[i]
                    print(
                        '...... GPR: y-Training data is standardized by shifting and scaling.')
    ...
    def gprTorch_1d_singleTask(self):
        ...
        # (0) Assignments
        # number of iterations in optimization of hyperparameters
        nIter = gprOpts['nIter']
        ...
        for i in range(nIter):
            ...
            if i == 0 or (i+1) % 100 == 0:
                print('...... GPR-hyperparameters Optimization, iter %d/%d - loss: %.3f - lengthsc: %.3f' %
    ...
    def gprTorch_pd_singleTask(self):
        ...
        for i in range(nIter):
            ...
            for j in range(p):
                lengthSc_.append(
                    model.covar_module.base_kernel.lengthscale.squeeze()[j].item())
            ...
            if i == 0 or (i+1) % 100 == 0:
                print('...... GPR-hyperparameters Optimization, iter %d/%d - loss: %.3f' %
                      ((i + 1), nIter, loss_), end="  ")
    ...
    def optim_conv_plot(self):
        ...
        if self.p == 1:
            ...
            for j in range(self.p):
                plt.plot(self.lengthSc[j], '-',
                         label='Lengthscale, param%d' % (j+1))
...
class gprPost:
    ...
    def torchPost(self):
        ...
        with torch.no_grad():
            ...
            # sdev of the posterior mean of f(q)
            post_sdev = (post_mean-lower_)/2.0
...
class gprPlot:
    ...
    def _set_vals(self):
        ...
        if 'labFS' in keys_:
            ...
            if len(self.labFS) != 2:
                raise ValueError(
                    "Value of 'labFS' should have length 2 (x,y-axes label fontsize).")
        if 'ticksFS' in keys_:
            self.ticksFS = self.pltOpts['ticksFS']
            if len(self.ticksFS) != 2:
                raise ValueError(
                    "Value of 'ticksFS' should have length 2 (x,y-axes label fontsize).")
        ...
        if 'save' in keys_ and self.pltOpts['save']:
            ...
            for L_ in list_:
                if L_ not in keys_:
                    raise KeyError(
                        "%s is required in pltOpts since 'save' is True." % L_)
                else:
    ...
    def _figSaver(self):
        ...
        DPI = fig.get_dpi()
        fig.set_size_inches(
            self.figSize[0]/float(DPI), self.figSize[1]/float(DPI))
    ...
    def torch1d(self, post_f, post_obs, xTrain, yTrain, xTest, fExTest, shift=0.0, scale=1.0):
        ...
        with torch.no_grad():
            ...
            post_obs_sdev_ = (
                (upper_obs.numpy() - lower_obs.numpy())/4.0)*scale
            ...
            plt.plot(xTrain, yTrain_, 'ok', markersize=4,
                     label='Training obs. y')
            plt.plot(xTest, post_f_mean_, '-r', lw=2, label='Mean Model')
            plt.plot(xTest, post_obs_mean_, ':m',
                     lw=2, label='Mean Posterior Pred')
            plt.plot(xTest, post_obs_sample_, '-k',
                     lw=1, label='Sample Posterior Pred')
            plt.fill_between(xTest, lower_f_, upper_f_,
                             alpha=0.3, label='CI for f(q)')
            plt.fill_between(xTest, lower_obs_, upper_obs_,
                             alpha=0.15, color='r', label='CI for obs. y')
            plt.legend(loc='best', fontsize=self.legFS)
            if hasattr(self, 'title'):
                plt.title(self.title, fontsize=self.titleFS)
            else:
                plt.title('Single-task GP, 1D parameter',
    ...
    def torch2d_2dcont(self, xTrain, qTest, fTestGrid):
        ...
        plt.clabel(CS1, inline=True, fontsize=15, colors='k',
                   fmt='%0.2f', rightside_up=True, manual=False)
        plt.legend(loc='best', fontsize=self.legFS)
        plt.xticks(fontsize=self.ticksFS[0])
        plt.yticks(fontsize=self.ticksFS[1])
    ...
    def torch2d_3dSurf(self, xTrain, yTrain, qTest, post_, shift=0.0, scale=1.0):
        ...
        xTestGrid1, xTestGrid2 = np.meshgrid(
            qTest[0], qTest[1], sparse=False, indexing='ij')
        ...
        upper_surf_obs = ax.plot_wireframe(
            xTestGrid1, xTestGrid2, upper_, linewidth=1, alpha=0.25, color='r')
        lower_surf_obs = ax.plot_wireframe(
            xTestGrid1, xTestGrid2, lower_, linewidth=1, alpha=0.25, color='b')
