...
def psobol_2d_test():
    ...
    def plot_trainData(n, fSamples, noiseSdev, yTrain):
        ...
        for i in range(500):  # only for plottig possible realizations
            ...
            plt.plot(x_, fSamples+noise_, '.',
                     color='steelblue', alpha=0.4, markersize=1)
    ...
    def trainDataGen(p, sampleType, n, qBound, fExName, noiseType):
        ...
        #  (c) Training response
        yTrain = analyticTestFuncs.fEx2D(
            xTrain[:, 0], xTrain[:, 1], fExName, 'comp').val
    ...
    def noiseGen(n, noiseType, xTrain, fExName):
        ...
        """
        if noiseType == 'homo':
            # noise standard deviation  (Note: non-zero, to avoid instabilities)
            sd = 0.5
            sdV = sd*np.ones(n)
        elif noiseType == 'hetero':
            sdV = 0.5 * \
                (analyticTestFuncs.fEx2D(
                    xTrain[:, 0], xTrain[:, 1], fExName, 'comp').val+0.001)
        return sdV
    #
    #----- SETTINGS
    ...
    xTrain, yTrain, noiseSdev, yTrain_noiseFree = trainDataGen(
        p, sampleType, n, qBound, fExName, noiseType)
    ...
    for i in range(p):
        ...
        if distType[i] == 'Norm':
            pdf.append(
                np.exp(-(q_-qInfo[i][0])**2/(2*qInfo[i][1]**2))/(qInfo[i][1]*mt.sqrt(2*mt.pi)))
        elif distType[i] == 'Unif':
            pdf.append(np.ones(nTest[i])/(qBound[i][1]-qBound[i][0]))
        else:
            raise ValueError(
                "distType of the %d-th parameter can be 'Unif' or 'Norm'" % (i+1))
        qTest.append(q_)
    # plot PDFs
    for i in range(p):
    ...
    for i in range(p):
        print('Main Sobol index S%d: mean=%g, sdev=%g', str(
            i+1), np.mean(Si_samps[i]), np.std(Si_samps[i]))
        print('Total Sobol index ST%d: mean=%g, sdev=%g', str(
            i+1), np.mean(STi_samps[i]), np.std(STi_samps[i]))
...
#    n=[100, 70, 80]       #number of samples for q1, q2, q3
    ...
    fEx_ = analyticTestFuncs.fEx3D(
        qTrain[:, 0], qTrain[:, 1], qTrain[:, 2], 'Ishigami', 'comp', {'a': a, 'b': b})
